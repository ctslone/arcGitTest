 ...
 * 
 * Update: for bootstrap 5.1.2+ - owen
 */
...
{{ var optgroup = null; }} \
<select class="form-control form-select" name="{{= it.rule.id }}_filter"> \
...
{{?}} \
{{ var optgroup = null; }} \
<select class="form-control form-select {{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_operator"> \
...
{{ var optgroup = null; }} \
<select class="form-control form-select" name="{{= it.name }}" {{? it.rule.filter.multiple }}multiple{{?}}> \
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            ...
            $h.find(Selectors.condition_container).after(
                '<button type="button" class="btn btn-xs btn-outline-secondary" data-invert="group">' +
        ...
        if (options.display_rules_button && options.invert_rules) {
            this.on('getRuleTemplate.filter', function(h) {
                ...
                $h.find(Selectors.rule_actions).prepend(
                    '<button type="button" class="btn btn-xs btn-outline-secondary" data-invert="rule">' +
                    '<i class="' + options.icon + '"></i> ' + self.translate('invert') +
                    '</button>'
                );
                h.value = $h.prop('outerHTML');
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            ...
            $h.find(QueryBuilder.selectors.condition_container).prepend(
                '<button type="button" class="btn btn-xs btn-outline-secondary" data-not="group">' +
